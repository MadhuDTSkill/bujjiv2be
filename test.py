...
BUJJI_SYSTEM_MESSAGE = (
                "You are Bujji, an AI developed by Madhu Bagamma Gari, a Python Full Stack Generative AI Developer. "
                "Bujji specializes in cosmology and space sciences, providing accurate and engaging responses based on scientific principles."
                "\n\n"
                "## Capabilities:\n"
                ...
                "- If a query involves speculation, Bujji provides scientifically accepted theories while avoiding unfounded claims.\n"
...
class SectionsOutput(BaseModel):
    sections: List[str] = Field(
        description="Relevant sections for this topic. The response will always contain general sections based on the user query. "
                    ...
                    "Bujji View (AI-generated perspective creatively inferred from knowledge)."
    )
...
class CosmologyQueryCheck(BaseModel):
    is_cosmology_related: bool = Field(
        description="Indicates whether the user query is specifically related to cosmology. "
                    ...
                    "If false, the query might be a greeting, farewell, or a general question about the chatbot (Bujji) itself, "
    ...
    response: str = Field(
        description="If the query is not related to cosmology:\n"
                    ...
                    "- For chatbot-related queries (e.g., 'Who are you?'), provide relevant details (e.g., 'I am Bujji, an AI assistant created by Madhu Bagamma Gari, specializing in cosmology and space sciences.').\n"
                    "- For completely unrelated questions (e.g., 'Who is Modi?'), politely inform the user that Bujji only focuses on cosmology (e.g., 'I focus only on cosmology-related topics. Let’s talk about the wonders of the universe!')."
    )
...
class BujjiWorkflow:
    ...
    def _build_workflow(self):
        ...
        builder.add_node('Bujji', self.bujji_node)
        builder.add_node('Bujji Soul', self.bujji_soul_node)

        builder.add_edge(START, 'Bujji')
        
        builder.add_conditional_edges(
            'Bujji',
            lambda state: "Dicussion Is Done" if state['is_discussion_done'] else "Need More Discussion",
            {
                "Need More Discussion" :  'Bujji Soul',
    ...
    def bujji_node(self, state: State) -> dict:
        # self._verbose_print("Bujji Node: Generating discussion response.", state)
    ...
    def bujji_soul_node(self, state: State) -> dict:
        # self._verbose_print("Bujji Soul Node: Continuing the discussion.", state)
    ...
    # def bujji(self, state : State):
    #     self._verbose_print(f"Bujji called with state: {state}")
...
# bujji = BujjiWorkflow(llm_instance, tool_llm_instance, verbose=True)
# answer = bujji.run("What is the black hole ?", selected_mode="Kids")
# print("\n\nBujji:", answer)
